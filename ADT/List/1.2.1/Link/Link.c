...
void InitList(Flist *list_address)
{
    ...
    *list_address = NULL;
    ...
    if(prev == NULL)//检查一下这个链表是不是第一个链表
        User_LoginList = reserve;
    else 
    {
...
//操作链表的节点
void OperateAllList(Flist * list_address, void(*pfunc)(Item * item))
{
    Node * scan = *list_address;
...
//对一个链表的节点进行排序
void ListSort(Flist * list_address, int(*rule)(Item item, Item itemaft))
{
    Node * prev, * after, * temporary, * middle;
    Flist Head = *list_address;
    if(*list_address == NULL) return ;
    enum Kbool setting;
    while(1)
    {
        setting = F;
